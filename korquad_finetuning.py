...
class QuestionDataSet(Dataset):
    ...
    def load_data(self):
        ...
        for line in question_lines:
            ...
            qa_tokens = ['<question>'] + tokenizer.tokenize(question) + ['</question>'] + ['<answer>'] + tokenizer.tokenize(answer) + ['</answer>']
...
torch.save(model.state_dict(), '/'.join(model_dir.split('/')[:-1]) + '/korquad_model.bin' )